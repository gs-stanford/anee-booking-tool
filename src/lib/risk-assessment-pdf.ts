...
export async function buildRiskAssessmentPdf(record: RiskAssessmentPdfRecord) {
  ...
  if (record.riskLevel === RiskAssessmentLevel.HIGH) {
    drawTextLine(state.page, "Adam Boies (PI) signature", MARGIN_X, signatureTop - 54 + 18, 11, bodyFont);
    state.page.drawLine({
    ...
    drawTextLine(
      state.page,
      "High-risk assessment: collect PI signature before the experiment proceeds.",
      MARGIN_X,
      signatureTop - 104,
      10,
      bodyFont,
    ...
    drawTextLine(
      state.page,
      "Use the PDF record for lab documentation and approvals as needed.",
      MARGIN_X,
      signatureTop - 68,
      10,
      bodyFont,
...
export async function buildBlankRiskAssessmentPdfTemplate() {
  const pdf = await PDFDocument.create();
  const headingFont = await pdf.embedFont(StandardFonts.HelveticaBold);
  const bodyFont = await pdf.embedFont(StandardFonts.Helvetica);
  ...
  drawTextLine(page, "The Boies Group Risk Assessment Form", MARGIN_X, y, 22, headingFont);
  drawTextLine(page, "Blank template for manual completion and signature.", MARGIN_X, y - 24, 11, bodyFont, rgb(0.42, 0.4, 0.37));
  y -= 50;

  const lineField = (label: string, width = CONTENT_WIDTH) => {
    drawTextLine(page, label, MARGIN_X, y, 11, headingFont);
    page.drawLine({
      start: { x: MARGIN_X, y: y - 14 },
      end: { x: MARGIN_X + width, y: y - 14 },
      color: rgb(0.22, 0.21, 0.19),
      thickness: 1
    });
    y -= 34;
  ...
  lineField("Title of project / experiment / activity");
  lineField("Location of experiment");
  lineField("Dates");
  lineField("Risk level");
  lineField("Assessor");
  lineField("Assessor email");

  drawTextLine(page, "Brief description / procedure", MARGIN_X, y, 11, headingFont);
  page.drawRectangle({
    x: MARGIN_X,
    y: y - 96,
    width: CONTENT_WIDTH,
    height: 82,
    borderColor: rgb(0.22, 0.21, 0.19),
    borderWidth: 1
  });
  y -= 116;

  drawTextLine(page, "Hazard assessment table", MARGIN_X, y, 11, headingFont);
  y -= 16;

  const columns = [36, 150, 130, 190, 86];
  const headers = ["#", "Hazard", "Effect", "Control measures", "Residual risk"];
  let x = MARGIN_X;
  ...
  headers.forEach((header, index) => {
    page.drawRectangle({
      x,
      y: y - 24,
      width: columns[index] ?? 0,
      height: 24,
      borderColor: rgb(0.22, 0.21, 0.19),
      borderWidth: 1
    });
    drawTextLine(page, header, x + 6, y - 16, 9, headingFont);
    x += columns[index] ?? 0;
  });

  for (let row = 0; row < 4; row += 1) {
    const rowTop = y - 24 - row * 50;
    let rowX = MARGIN_X;
    columns.forEach((columnWidth, index) => {
      ...
      if (index === 0) {
        drawTextLine(page, String(row + 1), rowX + 12, rowTop - 28, 10, bodyFont);
      }
      rowX += columnWidth;
    });
  }
  ...
  drawTextLine(page, "PPE / emergency / monitoring notes", MARGIN_X, y, 11, headingFont);
  page.drawRectangle({
    x: MARGIN_X,
    y: y - 92,
    width: CONTENT_WIDTH,
  ...
  drawTextLine(page, "Assessor signature", MARGIN_X, y, 11, headingFont);
  page.drawLine({
    start: { x: MARGIN_X, y: y - 16 },
    end: { x: MARGIN_X + 200, y: y - 16 },
    color: rgb(0.22, 0.21, 0.19),
    thickness: 1
  });
  drawTextLine(page, "Date", MARGIN_X + 240, y, 11, headingFont);
  page.drawLine({
    start: { x: MARGIN_X + 240, y: y - 16 },
    end: { x: MARGIN_X + 340, y: y - 16 },
    color: rgb(0.22, 0.21, 0.19),
    thickness: 1
  });

  drawTextLine(page, "Adam Boies (PI) signature if risk is high", MARGIN_X, y - 48, 11, headingFont);
  page.drawLine({
    start: { x: MARGIN_X, y: y - 64 },
    end: { x: MARGIN_X + 200, y: y - 64 },
    color: rgb(0.22, 0.21, 0.19),
    thickness: 1
  });
  drawTextLine(page, "Date", MARGIN_X + 240, y - 48, 11, headingFont);
  page.drawLine({
    start: { x: MARGIN_X + 240, y: y - 64 },
    end: { x: MARGIN_X + 340, y: y - 64 },
    color: rgb(0.22, 0.21, 0.19),
    thickness: 1
  });
  ...
  return {
    bytes: await pdf.save(),
    fileName: "risk-assessment-blank-form.pdf"
  };
}
